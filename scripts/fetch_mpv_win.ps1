...
try {
    ...
    $clientH = Get-ChildItem -Path $extractDir -Recurse -Filter client.h |
        Where-Object { $_.Directory.Name -eq 'mpv' } |
        Select-Object -First 1
    if (-not $clientH) {
        throw "Could not find mpv headers (include/mpv/client.h) in the archive."
    }
    $destIncludeMpv = Join-Path $OutDir "include\mpv"
    if (-not (Test-Path $destIncludeMpv)) {
        New-Item -ItemType Directory -Force -Path $destIncludeMpv | Out-Null
    }
    Copy-Item -Path (Join-Path $clientH.Directory.FullName '*') -Destination $destIncludeMpv -Recurse -Force

    Write-Host "mpv headers and runtime copied to $(Resolve-Path $OutDir)"
}
finally {
